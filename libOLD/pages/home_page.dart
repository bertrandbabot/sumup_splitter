...
class _HomePageState extends State<HomePage> {
  ...
  void _startWatcher() {
    ...
    if (currentSettings == null || !currentSettings.isConfigured) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Configure d’abord SumUp et l’imprimante')));
      return;
    }
  ...
  void _stopWatcher() {
    watcher.stop();
    setState(() => statusText = 'Surveillance arrêtée');
  }

  Future<void> _checkOnce() async {
    final currentSettings = settings;
    if (currentSettings == null || !currentSettings.isConfigured) return;

    setState(() => statusText = 'Vérification...');
  ...
  Future<void> _reprint(LocalOrder order) async {
    ...
    try {
      ...
      ).printOneTicketPerItem(order.items);
      */
      if (!mounted) return;
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Commande réimprimée')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Erreur réimpression: $e')));
    }
  }
        ...
        children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        OutlinedButton.icon(
                          ...
                          label: const Text('Rafraîchir liste'),
                        ),
                        TextButton.icon(
                          onPressed: _clearHistory,
                          icon: const Icon(Icons.delete_outline),
                          label: const Text('Vider historique'),
                        ),
                      ],
                    ),
                  ],
          ...
          Expanded(
            child: printedOrders.isEmpty
                ? const Center(child: Text('Aucune commande traitée'))
                : ListView.builder(
                    padding: const EdgeInsets.all(16),
